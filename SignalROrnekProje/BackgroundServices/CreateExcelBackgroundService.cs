...
using SignalROrnekProje.Hubs;
using SignalROrnekProje.Models;
using System.Data;
using System.Threading.Channels;

namespace SignalROrnekProje.BackgroundServices
{
    public class CreateExcelBackgroundService(Channel<(string userId, List<Product> products)> channel, IFileProvider fileProvider, IServiceProvider serviceProvider) : BackgroundService
    {
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            while (await channel.Reader.WaitToReadAsync(stoppingToken)) 
            {
                ...
                var newExcelFileName = $"product-list-{Guid.NewGuid()}.xlxs";
                ...
                using (var scope = serviceProvider.CreateScope())
                {
                    ...
                    await appHub.Clients.Users(userId).SendAsync("AlertComplateFile",$"/files/{newExcelFileName}",stoppingToken);
